...
export default component<LayoutProps, unknown, LayoutSlots>(({ slots, props, signal }) => {
    ...
    return () => {
        ...
        const parentLabel = mod?.parent === 'core' ? 'Core' : mod?.parent === 'server' ? 'Server' : mod?.parent === 'terminal' ? 'Terminal' : mod?.parent === 'deploy' ? 'Deploy' : mod?.parent === 'actors' ? 'Actors' : mod?.parent === 'zero' ? 'Zero' : 'Lynx';
        const groupHref = mod?.parent === 'lynx' ? '/lynx/modules' : `/${mod?.parent}/packages`;
        const groupLabel = mod?.parent === 'lynx' ? 'Modules' : mod?.parent === 'deploy' ? 'Adapters' : 'Packages';
