...
The components run against your app's `sigx`, so no core packages to list — with `sigx` present, the only extra peers are the styling libraries `daisyui` (≥ 5) and `tailwindcss` (≥ 4):
