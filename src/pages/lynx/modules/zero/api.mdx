---
title: "API reference"
description: "API reference for @sigx/lynx-zero — every export of the SignalX Lynx Zero module."
layout: module-docs
category: "Reference"
order: 40
---
...
What a theme author writes: core tokens required, `*-soft` optional. `softMix` defaults to `0.16`.

`staticCss` declares that the design system generated a stylesheet rule for this theme from the same registry data at build time — a `.theme-name { --color-*: …; background-color: …; color: … }` rule plus its `@media (prefers-color-scheme: …)` twin under `.lynx-zero.scheme-<variant>-<theme-name>`. `ThemeProvider` then lets the CSS engine resolve the palette instead of declaring it inline; `hasStaticCss(name)` reads the flag. Leave it unset for anything registered at runtime — a tenant palette fetched from a backend, or a theme derived with `extendTheme()`. Claiming `staticCss` for a theme whose CSS does not exist leaves the app with **no** palette at all.
