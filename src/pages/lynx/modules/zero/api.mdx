...
What a theme author writes: core tokens required, `*-soft` optional. `softMix` defaults to `0.16`. (The `staticCss` option was removed in 0.19 — themes ride inline CSS variables, so first-frame paint is correct without a build-time class.)
