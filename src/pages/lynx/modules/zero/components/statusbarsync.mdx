...
<p class="lede">A side-effect-only component that keeps the OS status / navigation bar tint legible against the active global theme — light theme to dark icons, dark theme to light icons. It renders a zero-size placeholder and is meant to be mounted once inside your root [`ThemeProvider`](/lynx/modules/zero/api#themeprovider).</p>
...
- [ThemeProvider](/lynx/modules/zero/api#themeprovider) — applies a theme's CSS vars to a subtree; mount `StatusBarSync` inside the root one.
