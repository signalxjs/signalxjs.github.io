...
- **Props** — see [`RichTextInputProps`](#richtextinputprops). All props are optional.
...
Platform notes: iOS + Android only. Backed by `UITextView` (iOS) / `EditText` (Android); it renders nothing meaningful under pure web/SSR. Most consumers use this component rather than the raw [`sigx-richtext`](#sigxrichtextattributes) tag.
