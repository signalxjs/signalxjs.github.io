---
title: "API reference"
description: "API reference for @sigx/lynx-richtext — every export of the SignalX Lynx Rich Text module."
layout: module-docs
category: "Reference"
order: 40
---
...
The runtime exports (`RichTextInput`, `RichTextMethods`) back the `sigx-richtext` element, which runs on **iOS, Android and web** (native elements on device, a `contenteditable` implementation on web in v0.19.0+). The helper functions and the types are platform-shared. Importing the package entry also registers the `sigx-richtext` tag in the JSX namespace.
...
Platform notes: shared type. The runtime behavior backing it runs on iOS, Android and web.
...
Platform notes: the `sigx-richtext` element is native on iOS/Android and a `contenteditable` field on web. Most consumers use the typed [`RichTextInput`](/lynx/modules/richtext/components/richtextinput) component instead of this raw tag. `LynxCommonAttributes` and `LynxEventHandler` come from `@sigx/lynx-runtime`.
