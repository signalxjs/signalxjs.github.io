...
`@sigx/lynx-richtext` runs on iOS, Android **and web** — native elements (`UITextView` on iOS, `EditText` on Android) and a real `contenteditable` field on web (as of 0.19). It requires **no runtime permissions** and **no** `Info.plist` / `AndroidManifest` entries — it is a plain in-app text field.
