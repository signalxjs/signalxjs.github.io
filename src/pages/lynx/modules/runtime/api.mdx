...
The runtime calls `method` with `{ ...params, [valueKey]: value }`.

Reach for it when per-frame state is natively backed and a style write cannot express it without a layout pass. The first consumer is `<list>`'s `setBottomInset`, behind the [`bottomInset`](/lynx/modules/list/api#bottominset) prop — a frame-synced inset under keyboards and dragged sheets.

Both call shapes match `useAnimatedStyle`: static registers once at setup, and the reactive form (`() => spec | null`) re-binds when the spec's signature changes, with `null` meaning unbind. The wire ops are `REGISTER_AV_METHOD_BINDING` / `UNREGISTER_AV_METHOD_BINDING`.
