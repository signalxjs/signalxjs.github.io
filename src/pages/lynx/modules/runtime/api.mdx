...
The imperative sibling of `useAnimatedStyle`. Binds a `MainThreadRef`'s element to a `SharedValue` and has the main-thread runtime invoke a **named UI method** on every flush boundary where the value changed — same dirty-diffing and derived-value ordering as a style binding, and no thread crossing per frame.

```ts
function useAnimatedMethod(
  elRef: MainThreadRef<MainThread.Element | null>,
  sv: SharedValue<unknown>,
  methodName: string,
...
interface AnimatedMethodSpec {
  sv: SharedValue<unknown>;
  methodName: string;                      // e.g. 'setBottomInset'
  valueKey?: string;                       // default 'value'
  params?: Record<string, unknown>;        // the SV value wins on collision
}
```

The runtime invokes `methodName` on the element with `{ ...params, [valueKey]: value }`. The invoke happens in the same flush pass as style bindings and **after** derived values fold, so binding a `useDerivedValue` result sees the fresh folded value in the same frame.

Reach for it when per-frame state is natively backed and a style write cannot express it without a layout pass. The first consumer is `<list>`'s `setBottomInset`, behind the [`bottomInset`](/lynx/modules/list/api#bottominset) prop.

**Parameters**

- `elRef` — the main-thread ref of the element to invoke on.
- `sv` — the shared value driving the invoke (static form).
- `methodName` — the native UI method (static form).
- `opts.valueKey` / `opts.params` — where the value is written and what is merged around it.
- `spec` — an accessor returning an `AnimatedMethodSpec` or `null` (reactive form). Use it when the shared value only becomes available after mount — a bottom sheet handing out its reveal SV via a callback prop on first render.
...
Two behaviours differ from a style binding:

- **A method binding stays dirty while its element is unmounted or unresolved**, and applies on a later flush. A style binding can tolerate a dropped frame because the next write re-applies — but a producer like the keyboard lift may settle once and never write again, and that value must not strand.
- **Failures are fire-and-forget.** An element whose host widget does not know the method degrades to a no-op — which is how web hosts without the native subclass behave.

The wire ops are `REGISTER_AV_METHOD_BINDING` / `UNREGISTER_AV_METHOD_BINDING`.
...
interface DerivedValueOptions {
  /** Publish each fold back to the background thread. Default `true`. */
  bridge?: boolean;
}
```
...
**`{ bridge: false }` for per-frame values.** The background mirror is not free: every fold dispatches an MT→BG event, so a derived value that changes per frame by construction — a gesture delta, a scroll offset — emits ~60–100 dispatches a second for a reader that does not exist. That alone can trip the engine's dispatch limiter (`ContextProxy::DispatchEvent called too frequently`, error 204) and red-box mid-gesture. Pass `{ bridge: false }` whenever the derived value is consumed **only** by main-thread bindings (`useAnimatedStyle`, `useAnimatedMethod`). The SV still exists and still folds every frame on the main thread; only the BG publish is skipped, so reading `.value` on the background thread stays safe but always returns the initial value.

```tsx
const dragPx = useDerivedValue([progress], 'scale', { factor: travelPx }, { bridge: false });
useAnimatedStyle(cardRef, dragPx, 'translateX');
```

`useDerivedValueReactive(accessor, opts?)` is the variant for a runtime-reactive factor — the accessor runs in an `effect` and re-registers on the **same** derived SV when sources or params change, so consumers never rebind. Returning `null` holds the last registration. It takes the same `DerivedValueOptions`.

### useFrameCallback

Runs a `'main thread'` worklet once per display frame — the per-frame API for a physics simulation, a game loop, an inertial settle, or a custom easing that has to see every frame. Also re-exported from the `@sigx/lynx` umbrella.
...
interface FrameCallback {
  ...
  stop(): void;
  /** Unregister permanently. Idempotent; called automatically on unmount. */
  dispose(): void;
}
...
interface FrameInfo {
  ...
  /** Milliseconds since this callback's previous frame. `0` on a run's first frame; clamped `>= 0`. */
...
The callback is registered once and driven by the main thread; it does not start until `start()` (or `autostart: true`). One shared `requestAnimationFrame` chain drives every registered callback, so N callbacks cost one host call per frame, all see the same `timestamp`, and the frame closes with a single element-tree flush. It ticks at the **display's** refresh rate — a 120 Hz device gets 120 ticks a second, not 60 — so size a timestep from `timeSincePreviousFrame`, never a constant. All timings derive from `Date.now()` deltas (there is no `performance.now()` on the main thread), so expect ~1 ms granularity: fine for driving an integrator, not a basis for sub-millisecond measurement. A callback that throws is contained and rate-logged, not auto-stopped. `dispose()` runs automatically on unmount; call it yourself for a module-scope loop.
...
const sim = useFrameCallback((frame) => {
  ...
  if (Math.abs(vel.current.v) < 1) stopFrameCallback(sim);   // MT → MT
});
...
const fling = Gesture.Pan()
  ...
  .onEnd(() => {
    ...
    startFrameCallback(sim);                                  // same frame, no round trip
  });

useAnimatedStyle(elRef, pos, 'translateX');
```

### startFrameCallback / stopFrameCallback

```ts
function startFrameCallback(fc: FrameCallback): void;   // 'main thread'
function stopFrameCallback(fc: FrameCallback): void;    // 'main thread'
```

Start or stop a frame callback from **inside another main-thread worklet** — a gesture's `onEnd`, a tap handler, or the frame callback itself. This is why activation is decoupled from registration: a flick's release velocity exists on the main thread, and the simulation should be running in that same frame rather than after a background round trip. Both are themselves `'main thread'` worklets: calling them from background code throws (the same contract as `measureViewportRect`). From the background thread use `fc.start()` / `fc.stop()` instead.
