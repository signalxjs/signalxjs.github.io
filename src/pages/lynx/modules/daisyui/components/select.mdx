...
`Select` is a controlled component. It does not own its value: pass the current selection through `value` and update your own signal in the `onChange` handler (see [Binding to state](#binding-to-state) below).
...
| `value` | `string` | &mdash; | The currently selected option's `value`. The matching option's `label` is shown on the trigger; if no option matches, `placeholder` is shown instead. |
