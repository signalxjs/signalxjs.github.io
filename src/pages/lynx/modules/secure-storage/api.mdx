...
```ts
const SecureStorage: {
  setItem(key: string, value: string, opts?: SecureStorageSetOptions): Promise<void>;
  getItem(key: string, opts?: SecureStorageGetOptions): Promise<string | null>;
...
Encrypted at-rest key-value storage backed by the iOS Keychain and, on Android, the Keystore plus EncryptedSharedPreferences. The CRUD vocabulary (`setItem` / `getItem` / `removeItem` / `clear`) is the same as [`@sigx/lynx-storage`](/lynx/modules/storage/api/), so code can swap between the two by durability need without renaming calls. Platform: both. Every method except `isAvailable` returns a Promise and rejects on native failure, with messages prefixed `[@sigx/lynx-secure-storage]`.
...
- **`opts`** — optional [`SecureStorageSetOptions`](#securestoragesetoptions). With `requireBiometric: true`, every future `getItem` of this key prompts for biometrics; `setItem` itself never prompts.
...
Options for [`setItem`](#securestoragesetitem). `requireBiometric` gates this individual key behind the OS biometric prompt on read. Platform: both.
...
Options for [`getItem`](#securestoragegetitem). `biometricPrompt` supplies the strings shown on the OS prompt for biometric-gated keys. Platform: both.
