...
```tsx
import { render, waitFor, findByText, within } from '@sigx/lynx-testing';

const { container } = render(<Picker />);

// however many turns the lazy load actually takes (default timeout 1000 ms)
const tabBar = await waitFor(() => container.findByType('tab-bar'));

// waitFor + getByText, scoped to a subtree
