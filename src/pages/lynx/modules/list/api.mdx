...
| `ListItemSnap` | type | `'start' \| 'center' \| 'end' \| { factor?: number; offset?: number; maxSnapCount?: number }` — see [`itemSnap`](#itemsnap). |
...
| `itemSnap` | `ListItemSnap` | Paginated snap: the alignment shorthand `'start'` / `'center'` / `'end'`, or `{ factor, offset, maxSnapCount }`. Omit the prop to disable snapping. See [below](#itemsnap). |
...
Sticky section headers (`sticky-top` / `sticky-bottom`) use the engine's sticky layout; `pluginSigxLynx({ enableNewSticky: true })` opts into Lynx 4.0's new implementation, which changes where and when a header pins — see the [plugin's page-config flags](/lynx/modules/plugin/api/#page-config-flags).
