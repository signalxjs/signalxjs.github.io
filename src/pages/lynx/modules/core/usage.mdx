...
// A genuinely synchronous native method — e.g. a constant the module exposes
export function documentDirectory(): string {
  guardModule('FileSystem');
  return callSync<string>('FileSystem', 'getDocumentDirectory');
}
```

Use `callSync` only for native methods that genuinely return synchronously (every `Clipboard` method above is async; `FileSystem.getDocumentDirectory` is a real sync one). Use `callAsync` for methods whose last native argument is a single result callback — it wraps that callback in a Promise that resolves with the callback's value.
