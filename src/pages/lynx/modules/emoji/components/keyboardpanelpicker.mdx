...
With an [`EmojiProvider`](/lynx/modules/emoji/api#emojiprovider) in scope the panel needs no `data` prop — it reads the shared dataset, search index, recents list and skin-tone preference, so a composer panel and a [`SheetPicker`](/lynx/modules/emoji/components/sheetpicker) stay in sync:
...
- [API reference](/lynx/modules/emoji/api#keyboardpanelpicker) — the typed `KeyboardPanelPickerProps`.
