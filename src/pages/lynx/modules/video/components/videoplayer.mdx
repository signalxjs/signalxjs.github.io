---
title: "VideoPlayer"
description: "VideoPlayer component for SignalX Lynx — usage, props and import (@sigx/lynx-video)."
layout: module-docs
...
## Resuming and observing native state

Set `startTime` to seek into a clip before the first play — handy for resuming where the viewer left off, or deep-linking to a moment. It is applied once per loaded `src`. To mirror the real playback state, listen for `onStateChange`: it reports `playing` / `paused` / `buffering` / `ended` transitions, including pauses driven by the OS or the platform controls overlay that the declarative `playing` prop can't observe.

```tsx
import { component, signal } from '@sigx/lynx';
import { VideoPlayer } from '@sigx/lynx-video';
import type { VideoPlaybackState } from '@sigx/lynx-video';

const Resume = component(() => {
  const state = signal<VideoPlaybackState>('paused');
  ...
  return () => (
    <view>
      <VideoPlayer
        ...
        startTime={90}
        autoplay
        onStateChange={(e) => state.set(e.detail.state)}
        style={{ width: '100%', aspectRatio: 16 / 9 }}
      />
...
| `startTime` | `number` | One-shot seek (in seconds) applied once the asset is ready, before the first play. Applied once per loaded `src` — for resume / deep-link. |
...
| `onStateChange` | `(e: VideoStateChangeEvent) => void` | Fires on `playing` / `paused` / `buffering` / `ended` transitions with `e.detail.{ state, positionMs }` — including OS- and controls-driven pauses. See [`VideoStateChangeEvent`](/lynx/modules/video/api#videostatechangeevent). |
