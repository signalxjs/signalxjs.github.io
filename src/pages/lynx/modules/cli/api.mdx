---
title: "API reference"
description: "API reference for @sigx/lynx-cli — every export of the SignalX Lynx CLI Plugin module."
layout: module-docs
category: "Reference"
order: 40
---
...
`--variant <name>` (or the `SIGX_VARIANT` env var) selects a build variant from the `variants` map — see [Build variants](/lynx/modules/cli/build-variants). See the [Usage guide](/lynx/modules/cli/usage) for command behavior and examples.
...
A build variant — a deep-partial of `LynxConfig` (override any field) plus control fields. Selected with `--variant <name>` (or `SIGX_VARIANT`); deep-merged onto the base, with the app id + display name auto-suffixed and the native project rendered into `android-<name>/` / `ios-<name>/`. See the [Build variants](/lynx/modules/cli/build-variants) guide.
...
The build-time orientation set. `'portrait'` / `'landscape'` lock to that axis; `'all'` is every orientation including upside-down portrait; `'default'` is the platform default (iOS: portrait + both landscapes; Android: `unspecified`). Overridable per platform under `android.orientation` / `ios.orientation`. Runtime locks (`Orientation.lock()` in `@sigx/lynx-core`) can only narrow this set — see [Orientation](/lynx/modules/cli/usage/#orientation).
