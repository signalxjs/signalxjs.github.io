...
- Sensible defaults are applied for everything you omit: `version` `1.0.0`, `buildNumber` `1`, `orientation` `portrait`, both platforms, Android `minSdk` 24 / `targetSdk` 35, iOS `deploymentTarget` `15.0`. Missing icon or splash images fall back to bundled placeholders.
...
`sigx dev` starts the rspeedy dev server, probes for a free port (default `8788`), and prints a QR code plus your LAN IP so a device on the same network can load the bundle.
...
- Device `console.*` logs stream back to your terminal; selected targets are auto-launched with the dev URL.
