---
title: "Usage"
description: "Using CLI Plugin in a SignalX Lynx app — @sigx/cli plugin patterns and examples with @sigx/lynx-cli."
layout: module-docs
category: "Guides"
order: 30
---
...
`orientation` declares the set of interface orientations the native hosts allow — `'portrait'` (the default) or `'landscape'` lock to that axis; `'all'` supports every orientation including upside-down portrait (iOS: all four `UIInterfaceOrientation*`; Android: `fullSensor`); `'default'` is the platform default (iOS: portrait + both landscapes; Android: `unspecified`). Override per platform under `android.orientation` / `ios.orientation`. It is rendered into `android:screenOrientation` and `UISupportedInterfaceOrientations`; existing projects pick a change up on the next `sigx prebuild`.

```ts
export default defineLynxConfig({
    name: 'My App',
    orientation: 'default',          // let the app rotate; runtime lock per screen
    ios: { requiresFullScreen: true }, // iPad follows the same set (otherwise all four)
});
```

Any value that allows more than one orientation means the app handles rotation **in place** — no Activity recreation, no bundle reload, app state preserved. Layout follows it through `useScreen()` / `useOrientation()` and CSS `@media (orientation: …)` queries, and a screen can narrow the set at runtime with `Orientation.lock()` / `useOrientationLock()` — but only *within* what this config declares: the build-time config is the ceiling, and a runtime lock outside it rejects. See [Screen metrics and orientation](/lynx/modules/core/api/#screen-metrics-and-orientation). On iPad, multitasking-capable apps must support all four orientations, so the lock only applies there with `ios.requiresFullScreen: true`.
...
The templates pin the Lynx engine at **4.0.1** on both platforms (PrimJS pinned explicitly at 4.0.0, the exact build Lynx 4.0.1 declares). That pin is the supported engine version for this release: the CSS at-rules, the engine memory query, `maxSnapCount` and the other 4.0 capabilities the modules rely on need it. When a release moves the pin, the upgrade is a **store-release-level** change:

1. Run `sigx prebuild --clean` — a plain `sigx prebuild` can take the fast path and leave the old engine in place.
2. Ship a new binary through the stores.
3. Only once the new binary is live, publish fresh OTA updates. The engine version is folded into the [OTA runtime fingerprint](/lynx/modules/updates/usage/#runtime-version-compatibility), so every update published against the previous host is invalidated by the bump, and the new binary will not accept them.

On iOS, XElement 4.0.1's default subspecs include `BlurView` and `WebView`, which the templates take by default, so the binary grows slightly.
...
**Run the two as a pair, in that order, every time.** Embedding is a build-output copy, not a generation step: it does not participate in prebuild's fingerprint fast path, so it never skips — but it can only copy what `sigx build` last produced. Embedding a bundle you did not just build is the failure this guards against, and it does not present as a failure: there is no crash and no error, the app launches and runs the *older* code, and it reads as "my change didn't work" rather than "my build is broken".

To verify an archive, compare `shasum dist/main.lynx.bundle` against the bundle extracted from it. Grepping the archive for a newly added symbol does **not** work — minification renames it. `sigx run:android --release` / `sigx run:ios --release` perform this check automatically and refuse to package on a mismatch; a hand-rolled pipeline should do the same.
