...
The native publisher also drives the Lynx engine's own colour scheme, so `@media (prefers-color-scheme: dark)` in app CSS resolves correctly on the first frame and re-resolves on a live flip — no JS round trip, no signal read. Use it next to the hook for anything that is purely presentational:

```css
.card { background-color: #ffffff; color: #111111; }
...
Requirements: a Lynx ≥ 4.0 host (the CLI templates pin 4.0.1) with the plugin's `enableCSSRule` on (the default). Live foreground flips need iOS 17+. On Android an OS scheme flip recreates the Activity unless the host adds `uiMode` to the Activity's `configChanges`. The web target does not carry `@media` rules into the bundle — keep the JS hook for web. See the [styling guide](/lynx/docs/styling/#at-rules-media-supports-font-face) for the other at-rules.
