...
The heavy-work sibling of `useFocusEffect`. Runs `cb` once the screen is focused **and** no transition is in flight — not before, and not on every focus regardless of motion; the returned cleanup runs on blur, on a new transition, or on unmount.
