---
title: "Usage"
description: "Using Navigation in a SignalX Lynx app — type-first native navigator patterns and examples with @sigx/lynx-navigation."
layout: module-docs
category: "Guides"
order: 30
---
...
## Transitions pre-stage the incoming screen

An animated push commits its render immediately but parks the incoming layer off-screen, and holds the slide until the background→main pipeline goes quiet: the screen's mount batch, post-mount effect flushes, native layout and a `<list>`'s initial cell pulls all complete while the outgoing screen is still on screen. Then the slide animates a fully built tree.

The effect is that a heavy destination slides in at vsync cadence instead of stuttering into place. It needs nothing from you — this is how animated transitions behave.

For work that must **not** run during the slide, there is an API:
[`useDidAppear()`](/lynx/modules/navigation/api#usedidappear) runs its callback only once the
screen is focused and no transition is in flight. `useFocusEffect` fires while the slide is
still running, and its op batch is applied on the same main thread driving the tween — so
expensive mount work scheduled on focus starves the animation it is running under.

What to expect in practice:

- **A short, capped delay before motion starts.** The settle window is bounded at 160 ms, so a screen that never goes quiet — a poller, a streaming feed — still transitions promptly, just with a best-effort pre-stage instead of a complete one.
- **Cards and modals park with a ~2px sliver on screen**, not exactly off it. A fully off-screen layer's texture is culled and never rasterized, which puts the whole screen's raster cost on the first animation frame; the sliver forces that raster into the settle window instead.
- **Pops get the same window** for the underneath layer's reveal relayout.
- **Sheet height semantics are unaffected** — [`useSheetHeight`](/lynx/modules/navigation/api#usesheetheight) reports the same values it always did.

The quiescence probe underneath is [`pendingOps()`](/lynx/modules/runtime/api#pendingops) from `@sigx/lynx-runtime`, paired with `waitForFlush()`.
