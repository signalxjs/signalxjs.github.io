...
`'surface'` is the iOS-style behaviour: dragging **collapses the sheet** until it is at its maximum, and only then does the content scroll — with a one-way handoff back. `<BottomSheet>` provides the ScrollDragHost, and an inner `@sigx/lynx` `<ScrollView>` adopts it automatically.
