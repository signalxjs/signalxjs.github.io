...
Plain (non-messaging) pushes keep replace-in-place, and below Android 6.0 stacking falls back to replace-in-place. The tap payload is the latest push's `data`, and `cancel(notification_id)` clears the whole stacked entry.
