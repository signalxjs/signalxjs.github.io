---
title: "Usage"
description: "Using Notifications in a SignalX Lynx app — local notifications patterns and examples with @sigx/lynx-notifications."
layout: module-docs
category: "Guides"
order: 30
---
...
- **Android** — for remote push, create a Firebase project and point [`android.googleServicesFile`](/lynx/modules/cli/usage#firebase-and-entitlements) at its `google-services.json`; prebuild then copies it into `android/app/` on every run and applies the Gradle plugin. Without it, the google-services plugin is skipped and the build still succeeds — add the config later and the next prebuild picks it up.
