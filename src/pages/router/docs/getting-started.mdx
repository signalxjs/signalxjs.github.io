...
The router runs against your app's copy of SignalX, so it expects `sigx` to already be installed — the reactive engine comes from there. No core packages to list: with `sigx` present, `pnpm add @sigx/router` is all you need.
