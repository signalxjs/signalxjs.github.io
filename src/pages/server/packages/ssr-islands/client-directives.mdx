...
`<div data-island>` placeholder in its place — no server HTML for the component
itself. The island still appears in `__SIGX_ISLANDS__` (so the client knows to
mount it), but with **no captured signal `state`**, since nothing ran server-side.
On the client it mounts **fresh**.
