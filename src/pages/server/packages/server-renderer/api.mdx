...
automatically unless the app already installed it or `serializeState: false` is
passed.
...
lower-level render APIs (install it on the app: `app.use(stateSerializationPlugin())`);
the `renderDocument*` family enables it by default.
