...
islands/selective/resumable hydration are layered on as `SSRPlugin`s.
...
Disable SSR head mode and return (and clear) the `HeadConfig`s collected since
...
The `ssr` object injected into every `ComponentSetupContext`. `ssr.load(fn)` runs
async data fetching on the server and is skipped on the client during hydration
(state restored from server). `isServer`/`isHydrating` flags.
