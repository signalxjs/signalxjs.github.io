...
keyed `useAsync`/`useStream` resolves inline (no placeholders or replacement
...
- **serializeState** — serialize resolved `useAsync`/`useStream` values for
...
keyed `useAsync`/`useStream` components and plugin streaming chunks before
...
`useAsync`/`useStream` to the client as `window.__SIGX_ASYNC__`, keyed by the
...
flags and any `serverState` to restore keyed `useAsync`/`useStream` values), runs
...
selective/island/resumable/Suspense hydration without core changes. Server hooks
...
to its rendered HTML once its keyed `useAsync`/`useStream` data resolves. Populated
...
data loading lives in the `useAsync`/`useStream` composables (from `sigx`), not
here.
