...
```ts
import { provideSSRPlugin, SSR_PLUGINS_TOKEN } from '@sigx/server-renderer/client';
...
- **`SSRPack`** — an `SSRPlugin` with an `install(app)` method; `app.use(pack)`
  ...
  the app so the render pipeline picks it up.
- **`reviveFromServer(...)`** — now public — revives a value serialized into the
  hydration blob (applying the same codec + custom handlers) on the client.
