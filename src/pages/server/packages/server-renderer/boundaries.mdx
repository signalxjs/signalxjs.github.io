...
Register it once and both the SSR state blob and [server functions](/server/docs/authoring) use it — pass it through the server plugin's `types` option (`serverPlugin({ types: [moneyHandler] })`). A registered handler then round-trips wherever the codec runs: `__SIGX_ASYNC__`, the boundary table, and server-function arguments and results.
