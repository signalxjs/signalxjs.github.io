...
The codec ships with handlers for `Date`, `Map`, `Set`, `bigint`, `URL`, `RegExp` and `undefined`. To carry a type of your own across the boundary, define a handler with `defineTypeHandler` from `@sigx/serialize`. Its `test` is a **type guard**, so it drives the `serialize` / `revive` types for you — no casts:
...
const moneyHandler = defineTypeHandler({
  ...
  serialize: (m) => ({ cents: m.cents, currency: m.currency }),
  revive: (e) => new Money(e.cents, e.currency),
});
```

A simple `instanceof` or discriminant `test` infers `T` cleanly; a **compound** predicate that TypeScript can't narrow may need an explicit type argument — `defineTypeHandler<Money>({ … })`.

Register the handler once and both the SSR state blob and [server functions](/server/docs/authoring) use it — pass it through the `@sigx/server/plugin` app plugin's `types` option:
...
A registered handler then round-trips wherever the codec runs: `__SIGX_ASYNC__`, the boundary table, and server-function arguments and results.

`reviveWithHandlers<T>(value, handlers)` applies the same codec manually when you already hold a serialized tree, and the bare `TypeHandler<T, Encoded>` interface is the non-inferring shape (`test(): boolean`) for handlers authored without the `defineTypeHandler` guard.
