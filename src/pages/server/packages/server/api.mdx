...
stubServerApp({ authenticate: () => ({ id: 'u_1', roles: ['staff'] }) });
...
`stubServerApp(config)` stamps the app seam for a test. Because the runtime is fail-closed, a `serverFn` under no app **denies with 401**, so a unit test that exercises a handler needs either a stubbed app or a `allowAnonymous: true` declaration.
