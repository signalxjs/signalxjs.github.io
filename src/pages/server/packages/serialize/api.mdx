...
Reading it back is unchanged — `reviveWithHandlers(JSON.parse(wire), handlers)` — once you have a string: like `JSON.stringify`, the result is `undefined` for a top-level symbol or function, so narrow it before parsing. There is no revive-from-string counterpart; that path is not hot.
