...
When what you actually want is the **string** — a storage adapter's `save`, a request body, a state blob — skip the intermediate tree with [`stringifyWithHandlers`](/server/packages/serialize/api#stringify--sigxserializestringify) from the opt-in `@sigx/serialize/stringify` subpath. It yields the same bytes as the pair above in one walk; reading back is unchanged:

```ts
import { stringifyWithHandlers } from '@sigx/serialize/stringify';

const wire = stringifyWithHandlers(value, [money]);        // string | undefined
if (wire === undefined) throw new Error('not serializable');  // a top-level symbol or function
const back = reviveWithHandlers<Cart>(JSON.parse(wire), [money]);
```
