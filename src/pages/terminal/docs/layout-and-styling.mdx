...
> The renderer draws a border only when `border` is truthy, and `"none"` counts as falsy — so `border="none"` and leaving the prop off are the same thing. Any *other* unrecognized value falls back to the single-line style.
>
> A borderless box also costs nothing: `padX` and `dropShadow` are drawn by the border pass, so on a bare `<box>` they are silently inert. [`boxChrome`](/terminal/docs/api#boxchrome) takes the `border` prop directly for exactly this reason.
