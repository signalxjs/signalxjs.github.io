...
`@sigx/terminal` is an umbrella package. Its own dependencies — the renderer (`@sigx/runtime-terminal`), the headless foundation (`@sigx/terminal-zero`) and the themed components (`@sigx/terminal-ui`) — install automatically, so you import `signal`, `component`, `defineApp`, the terminal intrinsics and every built-in component all from `@sigx/terminal`.

The SignalX core packages — `@sigx/reactivity` and `@sigx/runtime-core` (`0.6.x`) — are **peer dependencies**. Install them alongside the umbrella so every `@sigx/*` package shares one reactivity engine; otherwise a mismatched core copy would give you two engines whose signals and effects can't see each other. npm 7+ and pnpm with `auto-install-peers` add them automatically, but listing them explicitly works everywhere.
...
- [Dev mode](/terminal/docs/dev-mode) — HMR for terminal apps with `sigx-terminal-dev`.
