...
A fullscreen frame taller than the terminal is **clamped to the viewport**, the same as an inline one. Overflowing content is dropped from the bottom rather than shearing the screen, so a body that emits more rows than it was given degrades to a missing footer instead of a corrupted display. Size content to the space you have — see [`fitLines`](/terminal/docs/api#fitlines) and [`Shell`](/terminal/docs/components/shell).
