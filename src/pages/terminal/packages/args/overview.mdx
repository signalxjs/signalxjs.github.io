...
<p class="lede">`@sigx/args` is a fluent, type-aware command and argument parser for CLIs. The arg builders you chain drive the **types** your handler receives — declare `port: a.number().required()` and `ctx.args.port` is `number`, not `number | undefined`.</p>

<p class="mod-meta-row"><PkgVersion npm="@sigx/args" /> <span class="mmr-dot" /> Zero dependencies <span class="mmr-dot" /> MIT</p>

It has **zero runtime dependencies**, is platform-neutral, and is the parser behind
`@sigx/cli` and the lynx CLI plugins. It ships from the terminal monorepo
(`signalxjs/terminal`, `packages/args`) but is **fully standalone** — install it on
its own, with or without a terminal UI:
...
- [Terminal Dev](/terminal/packages/terminal-dev/overview/) — `sigx-terminal-dev` uses `@sigx/args` for its own flags.
