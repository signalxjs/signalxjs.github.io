...
- `skipLanguages` — fence languages the highlighter leaves untouched, so the raw `<pre><code class="language-…">` survives in the output for a later rehype plugin or island to claim — e.g. `['mermaid', 'math']`. [`@sigx/mermaid`](/mermaid/docs/ssg) is the ready-made consumer for the first of those.
