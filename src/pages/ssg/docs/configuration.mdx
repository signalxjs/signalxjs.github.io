...
- `spaNavigation` (default `true`) — internal anchor clicks (including plain markdown links in MDX) navigate through the router instead of full page reloads. Opt out per link with a `data-no-spa` attribute, or globally with `spaNavigation: false`.
- `sitemap` — `SitemapOptions` (`exclude`, `additionalUrls`, `defaultChangefreq`, `defaultPriority`), or `false` to skip sitemap.xml/robots.txt entirely. Pages with `robots: noindex` and the `/404` page are excluded automatically, frontmatter `date` becomes `<lastmod>`, and a `public/robots.txt` you ship is never overwritten.
...
- `navigation.sectionOrder` — per-category sort order for auto-generated sidebars, merged over the built-in defaults.
- `clientImports` — side-effect imports added to the generated client entry; the usual home for `'@sigx/ssg/styles.css'`, the base stylesheet for ssg-emitted markup (code windows, package-manager tabs, heading anchors, dual-theme Shiki).

The previously documented `vite`, `autoEntries`, and `ThemeConfig.css` options were never functional and have been removed.
...
Themes bundle layouts, components, and CSS in a package referenced by the config `theme` field. A theme's exported `config` can also contribute markdown plugins, head tags, CSS imports, and a default layout — your own config always wins. Layouts receive the whole site config as `props.site` (including the `site.nav`, `site.logo`, and `site.repo` branding fields), so themes render your branding instead of hardcoding their own.
...
Three hooks in `ssg.config.ts` extend the production build — the slots for search indexing, OG-image generation, link checking, redirects:
...
export default defineSSGConfig({
    hooks: {
        ...
        onPageRendered(page) {
            // page is the build result entry plus the final html
        },
        async postBuild(result, ctx) {
            // everything is on disk; result.pages lists every page with meta
        },
    },
});
```

The per-page hooks run concurrently and in no guaranteed order; `postBuild` runs exactly once. A hook that throws fails the build.
