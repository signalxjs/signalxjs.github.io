...
`@sigx/ssg` contributes three subcommands to the `sigx` CLI: `dev`, `build`, and `preview`. They are available when `@sigx/cli` (>= 0.4.0) is installed and an `ssg.config.{ts,js,mjs}` file exists in your project root (the plugin's `detect` step activates on that file).
...
```bash
npx sigx ssg build [--config <path>] [--verbose] [--drafts] [--concurrency <n>]
```
