...
**Call the slot optionally.** A layout whose default fill is missing renders nothing there rather than throwing, so one page without body content cannot take the build down. `slots.default?.()` is also what type-checks under core 0.15, which checks a slot fill against the slot it declares and makes the accessor optional at the call site.
