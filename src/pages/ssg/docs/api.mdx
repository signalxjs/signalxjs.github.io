...
- **options.trailingSlash** — `'always'` (the default) pushes the canonical trailing-slash URL for folder routes so SPA navigation matches the hard-load form; extensioned paths (e.g. `/sitemap.xml`) are pushed verbatim. `'never'` strips the slash instead. Set it from the same config value as the build.
