...
Shiki highlighting options. `transformers` (from `shiki`) are applied to every highlighted block, with the raw fence meta exposed as `options.meta.__raw`; `skipLanguages` leaves listed fences untouched for a downstream rehype plugin/island (e.g. `['mermaid', 'math']` — see [`@sigx/mermaid`](/mermaid/docs/ssg)); `defaultPackageManager` (default `'pnpm'`) chooses the first variant on package-manager fences; `triggerLabel` is the live-code "Try Live" button label.
...
Configuration for the [LLM-friendly output](/ssg/docs/configuration#llm-friendly-output-llmstxt). An area's `full` defaults to `false` (per-area `llms-full.txt` is opt-in); the global `transform` receives every page and can branch on `page.path`.
