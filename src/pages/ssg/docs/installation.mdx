...
- **`sigx@^0.14.0`** — the SignalX runtime.
- **`@sigx/router@^0.11.0`** — the router the generated client entry drives. Its version line does not mirror core's minor (router `0.11` pairs with core `0.14`) — read the peer range, not the version number.
- **`@sigx/server-renderer@^0.14.0`** — used for the SSR render and the hydration client plugin.
