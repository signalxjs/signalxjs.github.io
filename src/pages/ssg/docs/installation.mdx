...
- **`@sigx/router@>=0.9.0 <0.10.0`** — the router the generated client entry drives. Its version line no longer mirrors core's minor (router `0.9` pairs with core `0.12`) — read the peer range, not the version number.
- **`@sigx/server-renderer@>=0.12.0 <0.13.0`** — used for the SSR render and the hydration client plugin.
