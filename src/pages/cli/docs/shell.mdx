...
It is a **budget, not a reservation.** The renderer's layout is content-driven, so the shell cannot hold the box on a tab's behalf. A body that emits more rows pushes the status line off the bottom. Fit content to it — [`fitLines(lines, pane)`](/terminal/docs/api#fitlines) from `@sigx/terminal-zero` does exactly that in one call:
