...
- **`allowUnknownFlags`** — instead of erroring on an undeclared flag, collect the raw tokens into `ctx.unknownFlags` (an optional `string[]`, populated only when this is set — hence the `ctx.unknownFlags ?? []` above). The command decides whether and how to forward them; nothing is passed through automatically. Use it for wrapper commands that hand flags to an underlying tool. Without it, unknown flags remain an error (the default).
