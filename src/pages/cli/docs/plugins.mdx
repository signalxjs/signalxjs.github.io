...
Commands declare their arguments through the `args` map, using the fluent [`@sigx/args`](/terminal/docs/args) builders exposed as `a` (re-exported from `@sigx/cli/plugin`). Each entry is a builder you chain to describe its type, aliases, default, and help — and the chain drives the type your handler receives in `ctx.args`:
...
Chain refiners to shape them: `.alias('p', …)` (single characters become short flags like `-p`), `.default(v)`, `.required()`, `.multiple()` (repeatable → array), `.describe(text)`. Builders are immutable — each refiner returns a new builder. For the full builder/refiner reference see the [`@sigx/args` guide](/terminal/docs/args).
...
- [`@sigx/args` guide](/terminal/docs/args) — the fluent arg builders behind `PluginCommand.args`.
