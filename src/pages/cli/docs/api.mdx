...
Entry point of the `create` command, exported from `@sigx/cli/create`. The `sigx` binary calls it with the options it already parsed via `@sigx/args`; the bare `@sigx/create` shim (`npm create @sigx@latest`) calls it with no argument, in which case it parses `process.argv` itself. In a TTY it runs the [wizard](/cli/docs/create) on `@sigx/terminal`; in non-interactive mode — no TTY, `yes: true`, or a kind (`kind`, `type` or `preset`) together with a positional `name` — it scaffolds headlessly and calls `process.exit()` with the result code.

- **`opts`** — optional pre-parsed `CreateOptions`. Omitted, it reads `process.argv`.
  ...
  interface CreateOptions {
      ...
      /** Comma-separated on the command line. */
      features?: string;
