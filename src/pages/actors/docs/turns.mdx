...
It is tempting to picture a mailbox holding messages. There isn't one, and the difference
matters when you go looking for it.

Turns chain onto a promise. Each new turn attaches to the tail of that chain with `.then()`;
nothing is stored, nothing can be inspected, reordered, bounded or selectively received. The
`queued` number you see in [`host.activations()`](/actors/docs/lifecycle) and the
[ops endpoint](/actors/docs/ops-endpoint) is a **counter**, not the length of a structure you
could look inside.

So there is no priority lane to configure, no bounded-queue policy to tune, and no way to drop
or reorder pending work. If you need any of those, they belong in your own code in front of the
actor.
...
| Runs outside | Why |
|---|---|
| [Guards](/actors/docs/guards) | a slow auth check must not occupy the actor |
