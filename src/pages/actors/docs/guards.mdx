...
`methodAuthorize` replaces the actor-level policy for the methods it names; it does
...
both run the policy. You cannot bypass one by calling from the server.
`info.transport` (`'wire'` or `'in-process'`) tells them apart where that matters,
while `Type#method` identity stays the same on both.
...
ctx.principal        // null — by design, there is no request behind a task body
```

`ctx.principal` being null inside a task body is exactly why `job.principal`
exists. Read the snapshot, not the ambient identity.
