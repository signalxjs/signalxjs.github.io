...
### `Intl` is optional too

Renderer-neutrality is a type-level claim; this is the runtime half of it. Some JavaScript
engines a sigx app runs on do not ship `Intl` at all — lynx's PrimJS is the one you are most
likely to meet.

The default `lightweightFormatter` does not need it: interpolation, plurals and selects are
resolved without `Intl.PluralRules` or `Intl.NumberFormat`. So a lynx or terminal app gets
working translations on an engine with no `Intl` object.

What `Intl` buys, where it exists, is locale-correct number, date and currency formatting. If
you need that, supply a `formatter` that uses it — and check the engines you ship to before you
do.
