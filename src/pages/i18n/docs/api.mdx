...
| `formatter` | Defaults to `lightweightFormatter` — interpolation, plurals and selects with **no `Intl` dependency**, so it runs on engines that lack it. Supply your own to opt into `Intl`-backed number/date formatting. |
