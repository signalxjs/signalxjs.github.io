...
- `events` — one change event per key (`events.count`, `events.name`) with `(value, prev)` callbacks. Lazy: the deep watcher behind a key only runs while someone is subscribed — see [Events & Observability](/store/docs/events-and-observability).
...
How that selection drives the instance types is covered in [TypeScript](/store/docs/typescript).
...
- `action.onDispatching.subscribe(fn)` — before the action body, with the original args.
- `action.onDispatched.subscribe(fn)` — after the action, with `(result, ...args)`; for async actions, after the promise resolves, with the **resolved value**.
- `action.onFailure.subscribe(fn)` — on a sync throw or async rejection, with `(error, ...args)`.

Errors are the caller's to handle — never swallowed: sync actions re-throw, async actions return the **original promise**. The full story — overlapping invocations, optimistic updates, aborts, race patterns — lives in [Actions & Async](/store/docs/actions-and-async).

> Declare each action as a single function signature (union parameter types instead of overloads); the event subscriber types are inferred from that one signature. Rationale in [TypeScript](/store/docs/typescript).
...
Every `subscribe()` returns a `Subscription` with `unsubscribe()`. When to reach for custom events versus per-key state events versus action events — and how devtools discovers all of them — is the subject of [Events & Observability](/store/docs/events-and-observability).

## Lifetime

The third argument to `defineStore` is the instance lifetime — `'singleton' | 'scoped' | 'transient'`, defaulting to `'scoped'`. It is honored by the core DI layer (core `0.5.0`): singletons are shared per app, scoped instances respect `defineProvide` subtree overrides, transients are created per call. [Lifetimes & DI](/store/docs/lifetimes-and-di) covers each in practice, plus parameterized stores and the SSR contract.

## Composables

Because a store is "just" a setup function, reusable behavior is a plain function that takes the context — the built-in [`persist`](/store/docs/persistence) composable is the canonical example:
...
Writing your own composables — and when to use the global `onStoreCreated` plugin hook instead — is covered in [Composables & Plugins](/store/docs/composables-and-plugins).
...
For larger stores — multiple slices, stores composed of stores, undo/redo — see [Patterns](/store/docs/patterns).

## Next steps

- [Actions & Async](/store/docs/actions-and-async) — `pending`, lifecycle events, and error semantics in depth.
- [Events & Observability](/store/docs/events-and-observability) — the event mental model and devtools discovery.
- [Lifetimes & DI](/store/docs/lifetimes-and-di) — lifetimes in practice, `defineProvide`, SSR.
- [Using Stores in Components](/store/docs/using-in-components) — reading state in render, sharing instances, cleanup.
