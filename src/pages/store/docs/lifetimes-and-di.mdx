...
- **`defineProvide` creates the instance without arguments.** Parameterized setups and subtree provision don't mix: design stores meant for `defineProvide` to take no setup args, and seed the fresh instance through an action right after providing it:
...
Setup parameters after the context are forwarded through the factory call ([Using Stores in Components](/store/docs/using-in-components#parameterized-stores)). With a `'transient'` lifetime every call uses its own arguments — that's the natural pairing. For `'scoped'` and `'singleton'`, arguments are honored **at first creation only**:
