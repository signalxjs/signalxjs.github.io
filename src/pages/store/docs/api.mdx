...
- `defineState(state)` — creates deep reactive, signal-backed state and returns `{ state, signals, events, patch }`.
- `defineActions(actions)` — wraps action functions with `pending` and dispatch lifecycle events; see `StoreAction`.
- `defineEvents<EventMap>()` — a typed topic group (core `createTopicGroup`) keyed by your event map.
...
`$patch` throws on a state key that is not part of the public surface. `$events` watchers are refCounted — they run only while subscribed and never fire immediately on subscribe.
...
- [Persistence](/store/docs/persistence)
