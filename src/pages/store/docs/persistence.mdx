...
## Composing with SSR state transfer

`persist` covers device-local state; [`ssrState`](/store/docs/ssr-state) carries state from the server render to client hydration. They layer cleanly — call `ssrState` **first** so its synchronous seed lands before `persist`'s (possibly async) hydration overrides it with device-local data when present. See [composing with `persist()`](/store/docs/ssr-state#composing-with-persist) for the ordering.

## Next steps

- [SSR state transfer](/store/docs/ssr-state) — seed a store from the server render; composes with `persist`.
