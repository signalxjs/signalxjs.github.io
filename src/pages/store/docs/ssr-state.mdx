...
This is the store-shaped entry point to the **same** SSR state-transfer mechanism that core's [`useAsync` and `useStream`](/core/docs/data-loading) use. They share one wire format — `window.__SIGX_ASYNC__`, a prototype-pollution-safe, page-lifetime blob the server renderer emits — and `store:<name>` is simply the **store keyspace** within it. Component-level fetches use `useAsync`; store-owned state uses `ssrState`. The server half of both is owned by `@sigx/server-renderer`'s document renderers; see the [data-loading guide](/core/docs/data-loading) for the component side and the [server renderer guide](/server) for the document-rendering APIs.
...
- [Data loading (`useAsync` / `useStream`)](/core/docs/data-loading) — the component-level half of the same SSR transfer.
