...
The filtering is **per key, not per entry**. A tampered entry is sanitized rather than rejected wholesale, so its surviving legitimate keys still seed the slice — one poisoned key does not cost you the rest of the page's state. Nothing that survives can reach the prototype.
...
The shared default, the `scope` option, and the rich-value round-trip need `@sigx/store@0.11` or newer; per-key sanitizing of a tampered blob needs `0.13`. The current `@sigx/store` <PkgVersion npm="@sigx/store" /> pins the **0.15.x core line** (`@sigx/reactivity` / `@sigx/runtime-core` `^0.15.0`).
