...
When a store is disposed, all of its topics are destroyed — they unregister from the registry, their refCount watchers stop, and `$events` / action events stop firing. See [Using Stores in Components](/store/docs/using-in-components#cleanup-and-lifetime).
