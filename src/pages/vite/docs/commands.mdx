...
exposed through the package's `bin`, so `npx sigx-types` runs it from any project
that has `@sigx/vite` installed — no global install needed.
...
`dist/`, and the declaration files carry source maps — so "Go to Definition" in
your editor lands in the real source, not a `.d.ts` stub. No configuration is
