...
components declared with a tag name, so JSX gets intellisense for those tags. It's
exposed through the package's `bin` field, so `npx sigx-types` runs it from any
project that has `@sigx/vite` installed — no global install needed.
...
Every `@sigx/*` package ships its TypeScript `src/` alongside the compiled
`dist/`, and the `.d.ts` files carry declaration maps (`.d.ts.map`) — so "Go to
Definition" in your editor lands in the real source, not a `.d.ts` stub. No configuration is
needed; it works as soon as the package is installed.
