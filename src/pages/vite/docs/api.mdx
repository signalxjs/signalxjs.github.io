...
automatic JSX with import source `importSource` (default `sigx`) when `jsx` is
...
): CollectedAssets;   // { modulepreload: string[]; stylesheets: string[] }
```

Turns the Vite client manifest into `DocumentOptions.assets` — the
`<link rel="modulepreload">` and stylesheet set for the given client entries.
`entries` are manifest keys (source-relative ids such as
`'src/entry-client.tsx'`); one that isn't in the manifest is skipped with a
warning. URLs are deduped and prefixed with `base`.
...
The `assets` entry imports nothing — no `node:` builtins, no `process` — so it
is importable from a workerd, Deno or Bun entry. `@sigx/vite/ssr` re-exports it,
but that module also carries `createDevRequestHandler` and so imports `node:fs/promises`
and `node:path` at its top level; reach for `/ssr` in dev and `/assets`
everywhere else.
...
export interface LibBuildOptions {
    ...
    jsx?: boolean;
    /** The module the automatic JSX runtime imports from. Read only when `jsx` is enabled. @default 'sigx' */
    importSource?: string;
