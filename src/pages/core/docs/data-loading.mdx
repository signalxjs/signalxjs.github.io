...
`useAsync` runs an async fetcher and returns reactive `{ value, loading, error }` state. Reading any of those inside the render subscribes it, so the component re-renders as the request settles.
