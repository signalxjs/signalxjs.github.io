...
`useData` returns a reactive `AsyncState<T>` with a `state` of `idle | pending | ready | refreshing | errored`, the last-good `value`, a `hasValue` flag, the `error`, a `loading` flag (`state === 'pending'` only), a `match()` dispatcher, and a `refresh()` method. It is a discriminated union, so `if (q.state === 'ready')` or `if (q.hasValue)` narrows `value` to `T` with no cast.
...
`match()` is the type-safe way to render every state at once. The `ready` arm is the only route to a **present** `T`:
...
| `error(e, ctx)` | fetch failed | `ctx.retry` re-runs; `ctx.value`/`ctx.hasValue` are the last-good (for "keep content + toast"); omitted ⇒ bubbles to an [error scope](/core/docs/error-handling) |
| `ready(value)` | the happy path | **required** |

The error arm's second parameter is a **context object**, not positional arguments — destructure what you need:

```tsx code
error: (e, { retry }) => <Banner error={e} onRetry={retry} />,

// The last-good value, if the cell ever had one:
error: (e, ctx) => (
    <>
        <Banner error={e} onRetry={ctx.retry} />
        {ctx.hasValue && <Table rows={ctx.value} />}
    </>
),
```

You never have to use `match` — `state`, `value`, `error` and `loading` are plain reactive reads — but it keeps the four states exhaustive.

### An error keeps the last-good value

`value` survives a failed fetch. A cell that succeeded once and then errored still
holds its data, so content next to an error banner stays on screen instead of
blanking — the SWR posture. `'errored'` splits on `hasValue`: a cell that never
succeeded has `value: null`, `hasValue: false`.

Two consequences worth knowing:

- **`q.error && !q.value` no longer means "errored"**, because an errored cell may
  well have a value. To blank deliberately, check `q.state === 'errored'`.
- **Retrying from errored-after-success goes to `'refreshing'`, not `'pending'`** —
  no skeleton flash over content the user can still see.

### Presence is not the value

`value` is not a presence signal. A fetch that legitimately resolves `null` — a
"not found" read — has a value, and `ready` is the correct arm for it. Presence
is `state` (`'ready'` or `'refreshing'`) or the `hasValue` flag:

```tsx code
const profile = useData(() => ['profile', props.id], loadProfile);

// A nullable T reaches `ready` with null, and stays there across a refresh.
profile.match({
    pending: () => <Skeleton />,
    ready: (p) => p ? <Profile data={p} /> : <p>No profile yet</p>,
});

// The flag, when you're reading the cell directly rather than matching.
if (profile.hasValue) { /* value is what the fetcher returned, null included */ }
```

Because presence is tracked separately, a cell holding `null` moves to
`refreshing` on `refresh()` rather than back to `pending`, so the content on
screen survives a background revalidate.

`hasValue` is on every `AsyncState` — `useData` cells, [`@sigx/cache`](/core/packages/cache/overview) cells, `all()` and SSR-side state.

There is no need to wrap a nullable result in an envelope to survive SSR
transfer. The `__SIGX_ASYNC__` transfer is presence-based — own-key membership —
so a transferred `null` or `undefined` round-trips as a hit.
...
- **Runs on the server**, serializing the resolved value under the key into [`window.__SIGX_ASYNC__`](/core/docs/advanced/globals) — a prototype-pollution-safe, page-lifetime cache the server renderer emits. It is **readable from the browser console**, so return only what the page renders; see [what the page exposes](/core/docs/advanced/globals#what-the-page-exposes-is-yours-and-unfiltered).
...
A [server function](/server/packages/server/overview) already carries a stable identity, so it *is* a key. Pass one straight to `useData` and the function is both the key and the fetcher — no string to invent, no fetcher to write:
...
To refresh a server-function read after a write, declare what it invalidates **by reference** — a bare server-function reference prefix-matches every parameterized read of it, and the same declaration drives single-flight refresh of the server boundaries that rendered it. That declaration can live on the server function's own [options](/server/packages/server/usage) (its `invalidates`, which runs on every transport) or on a client action via [`@sigx/cache`](/core/packages/cache/overview); with neither, refresh the read explicitly with `.refresh()` (below).

### What invalidates reaches

An `invalidates` pattern matches a read's **canonical key**, and that is the
whole rule. A pattern is an exact key string, a tuple prefix, or a bare
server-function reference that matches every read of that function:

```ts
export const addItem = serverFn({
    input: ItemInput,
    handler,
    invalidates: () => [getCart],        // every useData(getCart) read
});
```

Every mounted `useData` read whose key matches is refetched — a read needs no
`cache` option to be reachable, and no cache pack has to be installed. A pattern
that matches nothing warns in development rather than failing silently.

> Reading and writing an **actor** works the same way, with its own pair of
> composables — `useActorState` and `useActorAction` from
> [`@sigx/actors`](/actors/) — built on these primitives, so a write
> invalidates the reads it staled without a manual `refresh()`. See
> [Reads & writes in components](/actors/docs/in-components).

Invalidation also drops matching keys from the `__SIGX_ASYNC__` transfer blob.
Keys transferred from SSR live for the page's lifetime and a fresh mount restores
from them as `ready` without fetching, so clearing them is what makes leaving a
route and coming back show the post-mutation value.
...
Everything above is core's *mechanism* — keyed reads, dedup, SSR transfer, and key-addressable refresh. Freshness and retention are **policy**, and policy lives in a pack. Add [`@sigx/cache`](/core/packages/cache/overview) and the same `useData`/`useAction` gain a `cache` option: `staleTime`, focus and interval revalidation, `keepPreviousData`, cache-aware `invalidate()` and optimistic `mutate()` — one plugin, no change to your call sites.
