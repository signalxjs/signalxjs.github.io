...
A slot declaration is a contract on both sides, and TypeScript enforces both
halves:
...
- **Calling a props-declaring slot with no arguments** — `slots.row?.()` where
  `row` declares props. Fix by passing them: `slots.row?.(props)`.
...
The payoff for correct code is inference: an unannotated fill parameter is typed
from the declaration, with no annotation and no cast.
