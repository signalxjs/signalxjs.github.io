...
Model props are available during **server** setup too, so `props.model.value`
reads the bound value inside `renderToString` exactly as it does on the client.
A component taking a model needs no SSR guard around it.
