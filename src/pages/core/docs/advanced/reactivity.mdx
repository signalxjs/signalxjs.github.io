...
Arrays key on `length` rather than a separate iteration dependency, since an
index write already triggers `length`. Maps and Sets have always tracked their
own iteration and `.size`.

Plain property access is unaffected: `state.foo` never goes through the traps
that enumeration uses, so the common path costs nothing extra.
