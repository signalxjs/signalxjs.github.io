...
value, and the cell stays `ready`. Read presence from `state` (`'ready'` or
`'refreshing'`) or from `hasValue`, which is present on `useData` cells,
`@sigx/cache` cells, `all()` and SSR-side state.
