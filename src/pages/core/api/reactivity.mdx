---
title: Reactivity
description: "API reference for SignalX reactivity — signal, computed, effect, batch and the fine-grained primitives that drive every SignalX render target."
layout: docs
category: Reference
order: 20
---
...
dependency `obj.x` does. Arrays carry that dependency on `length`; Maps and Sets
track iteration and `.size`. See [Enumerating Reactive
...
Called **directly in a component's setup body**, the effect is tied to that component and stops on unmount — the handle is there if you want it, not something you must keep. Created anywhere else (including inside `onMounted` or an async callback), stopping it is yours to do. See [Effect cleanup](/core/docs/effects#effect-cleanup).
...
Like [`effect`](#effect), a `watch` created directly in a component's setup body stops automatically on unmount; `pause()`/`resume()` remain useful in the meantime. See [Effect cleanup](/core/docs/effects#effect-cleanup).
