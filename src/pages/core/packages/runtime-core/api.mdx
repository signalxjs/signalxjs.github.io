---
title: "API reference"
description: "API reference for @sigx/runtime-core — every export of the SignalX Runtime Core package."
layout: module-docs
category: "Reference"
order: 40
---
...
### Host namespaces (RendererOptions)

`createRenderer(nodeOps)` takes a `RendererOptions` object — the host operations a
platform implements. Three of them are optional and concern **namespaces**.

Core is namespace-agnostic: it knows nothing about SVG. It threads an **opaque
boolean flag** through the tree walk, caches it on each vnode, and hands it back to
`createElement` and `patchProp` as the `ns` parameter. What that flag *means* is the
host's business — in `@sigx/runtime-dom` it means "in the SVG namespace".
...
interface RendererOptions<HostNode = any, HostElement = any> {
  ...
  patchProp(el: HostElement, key: string, prev: any, next: any, ns?: boolean, appContext?: AppContext | null): void;
  // …

  /** The flag for an element, given its parent's. `undefined` parentNS means the
   *  surrounding context is unknown (a hydrated subtree patched from the top) —
   *  hosts typically fall back to a tag-name heuristic there. */
  getElementNamespace?(tag: string, parentNS: boolean | undefined): boolean;

  /** The flag children inherit, given the element's own. Lets a host RESET the
   *  context at a boundary element (in the DOM, `foreignObject`). */
  getChildNamespace?(tag: string, ns: boolean): boolean;

  /** The flag of an element's CONTAINER, given its tag and cached flag. Used when
   *  replacing a node whose surrounding context is unknown. */
  getContainerNamespace?(tag: string, ns: boolean): boolean;
}
```

Defaults when a hook is **absent**:

| Hook | Absent behaviour |
|---|---|
| `getElementNamespace` | Every element resolves to the default namespace (`false`) |
| `getChildNamespace` | Children inherit the element's flag unchanged |
| `getContainerNamespace` | The container resolves to the default namespace (`false`) |

**For renderer authors:** a DOM-like custom renderer must implement these to keep
SVG working — core no longer detects SVG tags itself, so omitting them resolves
everything to the default namespace. `@sigx/runtime-dom`'s
[`nodeOps`](/core/packages/runtime-dom/api#nodeops) is the reference
implementation. Renderers with no namespace concept (terminal, lynx) simply omit
all three.

The `ns` parameter of `createElement`/`patchProp` was previously named `isSVG`;
positions are unchanged, so only the name differs.
...
The reactive result of `useData` (and, via `all`, the combined state), plus the `match` arm table, the shared fetcher shape, and the read options. `value` is the best data the cell has — kept across a same-key `refresh()` **and across a failed fetch**, cleared on key change; `loading` is `state === 'pending'` only; `refresh()` never rejects (failures land on `.error`).

`AsyncState<T>` is a discriminated union, so `state`/`hasValue` checks narrow `value` without a cast. Extending it with `interface X extends AsyncState<T>` no longer compiles — intersect instead (`type X<T> = AsyncState<T> & { … }`).
...
// Each member narrows state / value / hasValue / error / loading:
...
interface AsyncErrored<T> extends AsyncStateBase<T> {
  readonly state: 'errored';
  readonly error: Error;                     // splits on hasValue: a cell that
                                             // once succeeded still holds its value
  readonly loading: false;
}
