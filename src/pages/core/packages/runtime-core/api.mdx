...
- `subscribe` **throws on a destroyed topic** (previously it silently re-attached a handler that could never be cleaned up).
...
Lifetime options for `defineFactory` (and consumers like `@sigx/store`'s `defineStore`). A string-literal union — it replaces the old `InstanceLifetimes` enum, and it is enforced (see [defineFactory](#definefactory) for the semantics of each value).
