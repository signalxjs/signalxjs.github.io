...
Composable that runs an async fetcher and returns a reactive `{ value, loading, error, refresh }` state; the render re-runs as the state changes. The fetcher receives an `AbortSignal`. Pass an explicit string key to make the call SSR-transferable — it runs during SSR, serializes its value under the key into `window.__SIGX_ASYNC__`, restores on hydration without refetching, and dedupes per key.
...
- `key` — (keyed form) the cache/serialization key.
- `fetcher` — an async function producing the value; `ctx.signal` is an `AbortSignal`.
- `options.throwOnError` — throw the error when `.error` is read (routes to an error boundary). Default `false`.
- `options.server` — (keyed form) run the fetcher during SSR. Default `true`.
...
### AsyncState / AsyncFetcherContext / AsyncOptions

Reactive result of `useAsync`, plus the fetcher context and options. `value` and `error` are mutually exclusive; `refresh()` re-runs the fetcher stale-while-revalidate (keeps `value` while loading) and repopulates the keyed cache.
...
interface AsyncFetcherContext {
  signal: AbortSignal; // pass to fetch(url, { signal })
}

interface AsyncOptions {
  throwOnError?: boolean; // default false
  server?: boolean;       // keyed form, default true
}
```
