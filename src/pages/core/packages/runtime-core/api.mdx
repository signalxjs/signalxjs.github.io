...
### ModelBinding / EventDefinition / deprecated Define aliases

```ts
type ModelBinding<_T> = [object, string];
type EventDefinition<T> = { __eventDetail: T };
// DefineProp / DefineModel / DefineEvent / DefineSlot / DefineExpose are @deprecated
// flat aliases of the corresponding Define.* members — prefer the Define namespace.
```
