...
type AsyncState<T> =
  | AsyncIdle | AsyncPending | AsyncReady<T> | AsyncRefreshing<T> | AsyncErrored<T>;

interface AsyncStateBase<T> {
  readonly state: AsyncStateName;
  readonly value: T | null;                  // last-good; survives an error
  readonly hasValue: boolean;                // presence — a resolved null IS a value
