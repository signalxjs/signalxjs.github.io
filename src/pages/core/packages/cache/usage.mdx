...
const rename = useAction(renameUser, {
    cache: {
        optimistic: {
            key: 'user',
            apply: (current: { name: string } | null, next: { name: string }) =>
