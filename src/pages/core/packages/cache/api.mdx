...
| `optimistic.apply` | Produces the provisional value from `(current, input)`. Rolled back on failure — unless a newer write landed on the entry meanwhile. Both params infer as `unknown` (the cached state can't be inferred from the action), so **annotate them** — e.g. `apply: (current: User \| null \| undefined, next: Rename) => …`. `current` is nullish (`null`/`undefined`) when the target read has nothing cached yet. |
