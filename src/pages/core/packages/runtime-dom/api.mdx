---
title: "API reference"
description: "API reference for @sigx/runtime-dom — every export of the SignalX Runtime DOM package."
layout: module-docs
category: "Reference"
order: 40
---
...
<p class="lede">Exports of `@sigx/runtime-dom` <PkgVersion npm="@sigx/runtime-dom" />. Import from `@sigx/runtime-dom`; low-level renderer primitives also re-export from `@sigx/runtime-dom/internals`.</p>
...
**SVG lives here, not in core.** `@sigx/runtime-core` is namespace-agnostic — it threads an opaque boolean flag and asks the host what it means. `nodeOps` supplies the three [namespace host ops](/core/packages/runtime-core/api#host-namespaces-rendereroptions): the SVG tag-name set in `getElementNamespace`, the `foreignObject` reset in `getChildNamespace`, and the container rule in `getContainerNamespace`. It is the reference implementation for anyone writing a DOM-like custom renderer.
