...
This is the primitive that composables build teardown on: a `use*` helper registers its own cleanup with whatever scope calls it — no disposer argument to thread through, nothing to return.
