---
title: "Usage"
description: "Using Reactivity in a SignalX app — signals, computed & effects patterns and examples with @sigx/reactivity."
layout: module-docs
category: "Guides"
order: 30
---
...
## Enumerating a reactive object

Enumeration inside an effect or computed — `Object.keys()`, `for…in`, spread,
rest destructuring — subscribes to the object's **key set**, so adding or
deleting a key re-runs the reader:

```ts
import { signal, effect, batch } from '@sigx/reactivity';

const config = signal<Record<string, string>>({ theme: 'dark' });

effect(() => console.log(Object.keys(config).length));
// logs: 1

config.locale = 'sv';   // logs: 2
delete config.theme;    // logs: 1
```

The key-set dependency sits alongside the per-key ones. `Object.keys()` reads no
values, so it re-runs only when the key set changes; a spread copies every value,
so it re-runs on a value change too. `'theme' in config` subscribes to the same
dependency `config.theme` would.

Arrays carry their enumeration dependency on `length` instead, and Maps and Sets
keep their own iteration and `.size` tracking. Since every inserted key notifies
enumerating readers, bulk insertion belongs in a `batch`:

```ts
batch(() => {
    for (const [key, value] of entries) config[key] = value;
});
```
