...
so it re-runs on a value change too. `'x' in obj` subscribes to the same
dependency `obj.x` would.

Arrays key on `length` instead, and Maps and Sets keep their own iteration and
`.size` tracking. Since every inserted key notifies enumerating readers, bulk
insertion belongs in a `batch`:
