...
> **Peers.** Both packages peer-depend on `@sigx/reactivity` and `@sigx/runtime-core` `^0.14.0` (the `sigx` umbrella your app already runs on provides these). `@sigx/use-web` additionally peers `@sigx/use`. They never bundle their own copy of the core — a composable always shares the *same* reactive runtime as your components.
